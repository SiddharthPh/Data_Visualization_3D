            ...
            function loadModel( ) {
				...
				const loader = new GLTFLoader().setPath( '../../static/main/js/three.js-master/examples/models/gltf/' );
					loader.load( 'xyma13.glb', function ( gltf ) {
						console.log(gltf)
			...
			function updatetemp(geometry){
				var temps=[];
					...
					var j=1
					var k=5
					var sensortemp=[100,500,900]
				        for(var i=0;i <geometry.attributes.position.count;i++)
				        {
							if(geometry.attributes.position.array[j]==5.909872055053711)
							temp=32;
							else if(geometry.attributes.position.array[j]>4)
							temp=2
							else if(geometry.attributes.position.array[j]>1.9)
							temp=0
							else if(geometry.attributes.position.array[j]>-0.4)
							temp=sensortemp[0]
							else if(geometry.attributes.position.array[j]>-2)
							temp=sensortemp[1]
							else if(geometry.attributes.position.array[j]>-4.6)
							temp=sensortemp[2]
