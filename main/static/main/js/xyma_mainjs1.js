			...
			// scene.add( axesHelper );
				...
				earthDiv.style.marginTop = '-3.2em';
				// earthDiv.style.marginLeft='6em'
            ...
            function loadModel( ) {
				...
				const loader = new GLTFLoader().setPath( '../../static/main/js/three.js-master/examples/models/gltf/' );
					loader.load( 'xyma11.glb', function ( gltf ) {
						console.log(gltf)
			...
			function updatetemp(geometry){
				var temps=[];
					...
					var sensortemp=[100,200,400,600,700,800,900,80]
				        ...
				        {
							if(geometry.attributes.position.array[j]==6.630810737609863)
							temp=32;
							else if(geometry.attributes.position.array[j]>5)
							temp=2
							else if(geometry.attributes.position.array[j]>3.8)
							temp=0
							else if(geometry.attributes.position.array[j]>2.4)
							temp=sensortemp[0]
